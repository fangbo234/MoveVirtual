...
def add_to_parent(root_node,node,parent_id):
    if parent_id==root_node["idx"]:
        root_node["child"].append(node)
    else:
        for child_node in root_node["child"]:
            add_to_parent(child_node,node,parent_id)


def build_mixamo_tree():
    names = get_mixamo_names()
    root_node = {}
    idxs = []
    for name in names:
        if name[2] == -1:
            root_node["name"] = name[0]
            root_node["idx"] = name[1]
            root_node["child"] = []
            root_node["rotation"] = {"w":0,"x":0,"y":0,"z":0}
            root_node["position"] = { "x": 0, "y": 0, "z": 0}
            root_node["scale"] = {"x": 1, "y":1, "z": 1}
            idxs.append(name[1])
    for name in names:
        if name[1] in idxs:
            continue
            ...
            node["idx"] = name[1]
            node["child"] = []
            node["rotation"] = {"w": 0, "x": 0, "y": 0, "z": 0}
            node["position"] = {"x": 0, "y": 0, "z": 0}
            node["scale"] = {"x": 1, "y": 1, "z": 1}
            add_to_parent(root_node,node,name[2])
    return root_node
...
def mediapipe_to_mixamo(mp_manager,
    ...
    mixamo_json = build_mixamo_tree()
    print(video_path)
    #with open(mixamo_dict_string) as f:
    #    mixamo_json = json.load(f)
    #mixamo_json = json.load(mixamo_dict_string)
    is_find, hip_node = find_model_json(mixamo_json, Mixamo.Hips.name)
    if not is_find:
        return [False, None]
...
def mediapipe_to_mixamo2(mp_manager,
    ...
    # for hips move var
    _, model_right_up_leg = find_model_json(mixamo_bindingpose_json, Mixamo.RightUpLeg.name)
    __, model_right_leg = find_model_json(mixamo_bindingpose_json, Mixamo.RightLeg.name)
